...
Expo Application Services builds it in the cloud, signed with the key the
releases use:
...
That is the APK to publish: one built any other way cannot update over the one
people already downloaded.

There is also a local path for iterating without waiting on a queue, if the
...
It runs [`scripts/apk.ps1`](scripts/apk.ps1), which points Gradle at the SDK
(`ANDROID_HOME` plus `android/local.properties`, which `expo prebuild` wipes
every time), regenerates the native project when it is missing, and prints
where the `.apk` landed. `npm run apk:local:debug` builds the debug variant
instead, which needs `npx expo start --dev-client` running to have any
JavaScript.

The catch that makes it a development tool and not a release one: Expo's
template signs release builds with `debug.keystore`, not with the key EAS uses.
A locally built APK installs and works, but it cannot update over one downloaded
from the releases — uninstall first.
